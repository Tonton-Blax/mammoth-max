...
function DocumentConversion(options, comments) {
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.color || run.highlight) {
            var outColor = 'white';
            var inColor = tinycolor(run.color || run.highlight);
            var rgb = inColor.toRgb();
            var dominant = Math.max(rgb.r, rgb.g, rgb.b);
            if (dominant > 180) {
                if (dominant === rgb.r) {
                    outColor = Math.abs(rgb.g - rgb.r) < 20 ? 'yellow' : 'orangered';
                } else if (dominant === rgb.g) {
                    outColor = 'lime';
                } else {
                    outColor = "#0f62fe";
                }
                paths.push(htmlPaths.element('font', {color: outColor}, {fresh: false}));
            }
        }
        // if (run.highlight) {
        //     paths.push(htmlPaths.element('font', {'style': 'background-color:' + run.highlight}, {fresh: false}));
        // }
